...
class HardenedOrderManager:
    ...
    async def get_order(self, order_id: UUID) -> Optional[HardenedOrderEntry]:
        """Get order by ID."""
    ...
    async def get_orphaned_orders(self) -> List[HardenedOrderEntry]:
        ...
        return [
            entry for entry in self._orders.values()
                        ...
                        try:
                            ...
                            if broker_status != entry.state.name:
                                discrepancies.append({
                                    ...
                                    ).total_seconds() if entry.submitted_at else None,
                                })
