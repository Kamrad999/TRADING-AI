...
Fixed version compatible with existing Signal model.
"""
...
class InstitutionalSignalGenerator:
    ...
    def _create_signal(self, article: Article, entity: ExtractedEntity, event: ClassifiedEvent, 
        ...
        reason_parts = [
            ...
            f"event severity: {factors.event_severity:.2f}"
        ]
        ...
        return Signal(
            ...
            urgency=urgency,
            market_regime=MarketRegime.SIDEWAYS,  # Default market regime
            position_size=position_size,
            execution_priority=1,  # Default execution priority
            ...
            metadata={
                ...
                "event_severity": factors.event_severity,
                "keyword_strength": factors.keyword_strength,
                "entity_relevance": factors.entity_relevance,
                "reason": reason  # Store reason in metadata
            }
        )
    
    def _create_fallback_signal(self, article: Article, entity: ExtractedEntity, 
                                event: ClassifiedEvent, factors: SignalFactors, 
                                sentiment_score: float) -> Optional[Signal]:
