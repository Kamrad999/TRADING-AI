...
class TestDeterminismValidation:
    ...
    async def test_checksum_consistency(self):
        ...
        # Run twice
        engine1 = AcceleratedReplayEngine(
            bus, ReplayConfig(speed=ReplaySpeed.MAX_SPEED, seed=42)
        )
        result1 = await engine1.replay_historical_data(events)
        
        engine2 = AcceleratedReplayEngine(
            bus, ReplayConfig(speed=ReplaySpeed.MAX_SPEED, seed=42)
        )
...
class TestMarketRegimeScenarios:
    ...
    async def test_bear_market_scenario(self):
        ...
        print(f"✅ Bear market: Max DD tracked")
    ...
    async def test_flash_crash_scenario(self):
        ...
        print(f"✅ Flash crash: System survived")
...
class TestExecutionRealism:
    ...
    async def test_slippage_calculation(self):
        ...
        # Small order
        small = await simulator.simulate_fill(
            "AAPL", "buy", Decimal("100"), "market", market
        )
        
        # Large order
        large = await simulator.simulate_fill(
            "AAPL", "buy", Decimal("10000"), "market", market
        )
        
        # Larger orders should have more slippage
        assert large.slippage >= small.slippage
        
        print(f"✅ Slippage: Small {float(small.slippage)*10000:.1f}bps, Large {float(large.slippage)*10000:.1f}bps")
    ...
    async def test_partial_fill_simulation(self):
        ...
        for _ in range(10):
            fill = await simulator.simulate_fill(
                "AAPL", "buy", Decimal("1000"), "market", market
            )
            fills.append(fill)
